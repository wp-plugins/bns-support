...
function display_pluginsused($type, $display = false) {
	...
	if($type == 'stats') {
        ...
        $temp .= '<p class="bns-support-active-plugin-item"><strong><a href="'.$active_plugins['Plugin_URI'].'" title="'.$active_plugins['Plugin_Name'].' '.$active_plugins['Version'].'">'.$active_plugins['Plugin_Name'].' '.$active_plugins['Version'].'</a></strong><br />by '.$active_plugins['Author'].' (<a href="'.$active_plugins['Author_URI'].'" title="'.$active_plugins['Author'].'">'.__('url', 'wp-pluginsused').'</a>)</p>';
			}
		}
	} else{
